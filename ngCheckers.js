    ...
    $scope.newGame = function() {
      ...
      for (var i = 0; i < BOARD_WIDTH; i++) {
        ...
        for (var j = 0; j < BOARD_WIDTH; j++) {
          ...
          } else if ((i === BOARD_WIDTH - 3 && j % 2 === 1) || (i === BOARD_WIDTH - 2 && j % 2 === 0) || (i === BOARD_WIDTH - 1 && j % 2 === 1)) {
...
angular.module('ngCheckers', [])
  .controller('checkersCtrl', function ($scope, $timeout) {
    ...
    var countdown; // Initialize countdown variable

    function updateTimer() {
      timerDisplay.textContent = countdown;
      if (countdown <= 0) {
        // Handle game over logic when the timer reaches zero
        gameOver(null); // Pass null to indicate a stalemate or another appropriate action
        clearInterval(timerInterval);
      }
    }

    function startTimer() {
      countdown = 60; // Set the initial countdown time
      updateTimer();
    ...
    $scope.newGame = function () {
      // ... your existing newGame logic ...
      countdown = 0; // Initialize countdown to 0
      updateTimer();
    };
...
let timerInterval;
let countdown = 90; // Initial countdown time in seconds
...
function updateTimer() {
    ...
    if (countdown === 0) {
        ...
        clearInterval(timerInterval);
        alert("Game over! Red Score: " + redScore + " | Black Score: " + blackScore);
...
// Modify the makeMove function to update scores
function makeMove(number) {
    // ... Your existing code ...

    // Update scores based on your game logic
    if (turn) {
        redScore += 10;
    } else {
        blackScore += 10;
